				...
				<div class="col-12">
					...
					<table class="table shopping-summery">
						...
						<tr>
								...
								<td><?php $pi=explode(",",$fet['pppic']); ?>
								<img src="<?php echo "../Admin/img/".$pi[0]; ?>" alt="">
								</td>
								...
								<td class="qty" data-title="Qty"><!-- Input Order -->
									<div class="input-group input1">
										<input type ="hidden" name="catid" class ="cart_id" value="<?php echo $fet[`pad`];?>">
										<input type="hidden" name="price" class="ptprice" value="<?php echo $fet[`psale`];?>">
										<div class="button minus">
											<button type="button" class="btn btn-primary btn-number"data-type="minus" data-field="quant<?php echo $abc;?>">
												<i class="ti-minus"></i>
											</button>
										</div>
										<input type="text" name="quant[<?php echo $abc;?>]" class="input-number quty" id="num1"  data-min="1" data-max="100" value="<?php echo $fet[`quantity`];?>">
								...
								<td class="total-amount" data-title="Total"><span><?php echo $fet['psale'] ?></span></td>
								<td class="action " data-title="Remove" ><i data-pad="<?php echo $fet['pad']; ?>" class="ti-trash remove-icon delete"></i></a></td>
							</tr>
							<?php
...
<script>
	$(document).ready(function(){
		...
		$(".input").on("change",function(){
			var dat=$(this).closest("input1");
			var cartid =data.find(".cart_id").val();
			var ptprice=data.find(".ptpricee").val();
			var quty=data.find(".quty").val();
			alert(cartid);
			alert(ptpricee);
			alert(quty);
			$.ajax({
				url:"./ajax/cat.php",
				method:"POST",
				data:{mycartid: cartid, myprice:pricee, cqty:qnty,},
				success: function (res){
					alert(res);
					if(res==9){
						location.reload(true);
